  ...
  return <div className="flex flex-col items-center">
      <div className="relative">
        <Avatar className={`${sizeClass[size]} border-2 border-gray-200`}>
          <AvatarImage src={displayedImage} onError={e => {
        ...
        <div className="absolute -bottom-2 -right-2 flex gap-1">
          <Button type="button" size="icon" variant="secondary" className="h-8 w-8 rounded-full shadow" onClick={handleButtonClick} disabled={isUploading}>
            <Camera size={16} />
            <span className="sr-only">Upload image</span>
          </Button>
          
          {displayedImage && <Button type="button" size="icon" variant="destructive" className="h-8 w-8 rounded-full shadow" onClick={handleRemoveImage} disabled={isUploading}>
              <X size={16} />
