  ...
  const fetchConsultantDetails = async () => {
    ...
    try {
      const {
        data: consultant,
        error
      } = await supabase.from('consultants').select('location, languages, hourly_rate').eq('id', user.id).single();
      if (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isConsultant) {
        ...
        } = await supabase.from('consultants').select('id').eq('id', user.id).single();
  ...
  return <div className="space-y-6">
      <Card>
        ...
        <CardContent className="flex justify-center">
          <ProfileImageUploader currentImage={profileImage} onImageChange={uploadProfileImage} size="lg" />
        </CardContent>
      </Card>
      ...
      {isConsultant && <>
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="languages" value={languages.join(', ')} onChange={e => setLanguages(e.target.value.split(',').map(lang => lang.trim()).filter(Boolean))} placeholder="e.g., English, Spanish, French" />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="hourly-rate">Hourly Rate ($)</Label>
                <Input id="hourly-rate" type="number" value={hourlyRate} onChange={e => setHourlyRate(Number(e.target.value))} min="0" step="5" />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <TagsManager tags={tags} onChange={setTags} maxTags={6} />
            </CardContent>
          </Card>
        </>}
      ...
      <Card>
        ...
        <CardContent>
          <textarea className="flex min-h-[120px] w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring" placeholder="Write a short bio..." value={bio} onChange={e => setBio(e.target.value)} />
        </CardContent>
      </Card>
