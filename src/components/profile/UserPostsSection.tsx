...
interface UserPostsSectionProps {
  userId: string;
  userName: string;
  currentUserId?: string; // To determine if viewing own profile
}

export function UserPostsSection({ userId, userName, currentUserId }: UserPostsSectionProps) {
  ...
  const { data: posts, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<UserPost[]> => {
      let query = supabase
        .from('stories')
        .select('id, title, snippet, cover_image, tags, tag_type, published_at, is_featured, is_anonymous')
        .eq('author_id', userId)
        .eq('status', 'published');
      
      // If not viewing own profile, exclude anonymous stories
      if (!isOwnProfile) {
