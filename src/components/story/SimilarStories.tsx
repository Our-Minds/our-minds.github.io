...
export function SimilarStories({
    ...
    const fetchSimilarStories = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('stories').select('*').eq('tag_type', currentStory.tag_type).neq('id', currentStory.id).limit(5);
  ...
  return <ScrollArea className="h-full">
      <div className="p-3 space-y-3">
        {similarStories.map((story, index) => <Link key={story.id} to={`/story/${story.id}`} className="block group">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-white text-xs py-0.5 px-2 rounded-full font-medium flex-shrink-0 bg-[#037004]">
                      {story.tags[0]}
                    </span>
                  </div>
                </div>
